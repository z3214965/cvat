...
export default function implementAPI(cvat: CVATCore): CVATCore {
    ...
    implementationMixin(cvat.server.share, async (...args: Parameters<typeof serverProxy.server.share>) => {
    ...
    implementationMixin(cvat.server.register, async (...args: Parameters<typeof serverProxy.server.register>) => {
        const result = await serverProxy.server.register(...args);
        return result;
    });
    implementationMixin(cvat.server.login, async (...args: Parameters<typeof serverProxy.server.login>) => {
    ...
    implementationMixin(
        ...
        async (...args: Parameters<typeof serverProxy.server.changePassword>) => {
            await serverProxy.server.changePassword(...args);
        },
    );
    implementationMixin(
        cvat.server.requestPasswordReset,
        async (...args: Parameters<typeof serverProxy.server.requestPasswordReset>) => {
            await serverProxy.server.requestPasswordReset(...args);
        },
    );
    implementationMixin(
        cvat.server.resetPassword,
        async (...args: Parameters<typeof serverProxy.server.resetPassword>) => {
    ...
    implementationMixin(cvat.server.healthCheck, async (...args: Parameters<typeof serverProxy.server.healthCheck>) => {
        const result = await serverProxy.server.healthCheck(...args);
        return result;
    });
    implementationMixin(cvat.server.request, async (...args: Parameters<typeof serverProxy.server.request>) => {
    ...
    implementationMixin(
        cvat.assets.create,
        async (...args: Parameters<typeof serverProxy.assets.create>): Promise<SerializedAsset> => {
            const [file, guideId] = args;
            if (!(file instanceof File)) {
                throw new ArgumentError('资源需要上传一个文件');
            }
    ...
    implementationMixin(
        cvat.projects.get,
        async (filter: Parameters<CVATCore['projects']['get']>[0]): ReturnType<CVATCore['projects']['get']> => {
            checkFilter(filter, {
                id: isInteger,
                page: isInteger,
                pageSize: isInteger,
    ...
    implementationMixin(cvat.projects.searchNames, async (search, limit) =>
    ...
    implementationMixin(cvat.organizations.activate, (organization) => {
        checkObjectType('组织', organization, null, { cls: Organization, name: 'Organization' });
        config.organization = {
    ...
    implementationMixin(cvat.organizations.acceptInvitation, serverProxy.organizations.acceptInvitation);
    implementationMixin(cvat.organizations.declineInvitation, serverProxy.organizations.declineInvitation);
    ...
    implementationMixin(
        cvat.analytics.quality.reports,
        async (
    ...
    implementationMixin(cvat.analytics.quality.conflicts, async (filter: QualityConflictsFilter) => {
        ...
        const frames = Array.from(new Set(conflicts.map((conflict) => conflict.frame))).sort((a, b) => a - b);
        ...
        for (const frame of frames) {
            ...
            for (const objectConflicts of Object.values(conflictsByObject)) {
                if (objectConflicts.length === 1) {
                    ...
                    const mainObjectConflict =
                        objectConflicts.find((conflict) => conflict.severity === ConflictSeverity.ERROR) ||
                        objectConflicts[0];
                    ...
                    for (const objectConflict of objectConflicts) {
                        if (objectConflict !== mainObjectConflict) {
                            ...
                            for (const annotationConflict of objectConflict.annotationConflicts) {
                                if (
                                    !mainObjectConflict.annotationConflicts.find(
                                        (_annotationConflict) =>
    ...
    implementationMixin(
        cvat.analytics.events.export,
        async (filter: AnalyticsEventsFilter): ReturnType<CVATCore['analytics']['events']['export']> => {
            checkFilter(filter, {
                orgId: isInteger,
                userId: isInteger,
                jobId: isInteger,
