...
export default class ObjectState {
    ...
    constructor(serialized: SerializedData) {
        if (!isEnum.call(ObjectType, serialized.objectType)) {
            throw new ArgumentError(`ObjectState 必须提供有效的 objectType，传入了错误值：${serialized.objectType}`);
        }

        if (!(serialized.label instanceof Label)) {
            throw new ArgumentError(`ObjectState 必须提供正确的 Label，传入了错误值：${serialized.label}`);
        }

        if (!Number.isInteger(serialized.frame)) {
            throw new ArgumentError(`ObjectState 必须提供正确的帧号，传入了错误值：${serialized.frame}`);
        }
                ...
                points: {
                    ...
                    set: (points) => {
                        if (!Array.isArray(points) || points.some((coord) => typeof coord !== 'number')) {
                            throw new ArgumentError('Points必须为数字数组');
                        }
                        ...
                        if (data.shapeType === ShapeType.SKELETON) {
                            ...
                            if (points.length !== currentPoints.length) {
                                throw new ArgumentError(
                                    `尝试为骨骼设置错误的点数(${points.length} 与 ${currentPoints.length} 不匹配)`,
                                );
                            }
                ...
                rotation: {
                    ...
                    set: (rotation) => {
                        if (typeof rotation === 'number') {
                            ...
                            throw new ArgumentError('旋转角度必须为数字。');
                        }
                    },
                },
                group: {
                ...
                descriptions: {
                    ...
                    set: (descriptions) => {
                        if (
                            ...
                            throw new ArgumentError(`描述必须为字符串数组，实际得到：${data.descriptions}`);
                        }
    ...
    value: function deleteImplementation(frame: number, force: boolean): boolean {
        if (this.__internal && this.__internal.delete) {
            if (!Number.isInteger(+frame) || +frame < 0) {
                throw new ArgumentError('帧参数必须为非负整数');
            }
