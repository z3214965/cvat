        ...
        if (flush) {
            ...
            const retryIf504Status = async (
                ...
                if (error instanceof ServerError && error.code === 504) {
                    setTimeout(() => {
                        // just for logging
                        throw new Error(`在 ${action} 对象时从服务器收到代码504，正在运行备用方案`);
                    });
