...
export default class PluginRegistry {
    ...
    static async register(plug) {
        ...
        if (typeof plug !== 'object') {
            throw new ArgumentError(`插件必须为对象类型，实际得到："${typeof plug}"`);
        }

        if (!('name' in plug) || typeof plug.name !== 'string') {
            throw new ArgumentError('插件必须包含 name 字段，且类型为字符串');
        }

        if (!('description' in plug) || typeof plug.description !== 'string') {
            throw new ArgumentError('插件必须包含 description 字段，且类型为字符串');
        }

        if ('functions' in plug) {
            throw new ArgumentError('插件不允许包含 functions 字段');
        }
