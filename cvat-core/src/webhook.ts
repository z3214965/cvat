        ...
        Object.defineProperties(
            ...
            Object.freeze({
                ...
                targetURL: {
                    ...
                    set: (value: string) => {
                        if (typeof value !== 'string') {
                            throw new ArgumentError(`targetURL属性必须为字符串类型，尝试设置的类型为 ${typeof value}`);
                        }
                ...
                events: {
                    ...
                    set: (events: string[]) => {
                        if (!Array.isArray(events)) {
                            throw new ArgumentError(`事件列表必须为数组类型，尝试设置的类型为 ${typeof events}`);
                        }
                        events.forEach((event: string) => {
                            if (typeof event !== 'string') {
                                throw new ArgumentError(`事件项必须为字符串类型，尝试设置的类型为 ${typeof event}`);
                            }
                        });
                        data.events = [...events];
                    },
                    ...
                    set: (value: WebhookContentType) => {
                        if (!isEnum.call(WebhookContentType, value)) {
                            throw new ArgumentError(
                                `Webhook内容类型必须是WebhookContentType枚举成员，传入了无效值：${typeof value}`,
                            );
                        }
                ...
                description: {
                    ...
                    set: (value: string) => {
                        if (typeof value !== 'string') {
                            throw new ArgumentError(`描述属性必须为字符串类型，尝试设置的类型为 ${typeof value}`);
                        }
                ...
                secret: {
                    ...
                    set: (value: string) => {
                        if (typeof value !== 'string') {
                            throw new ArgumentError(`密钥属性必须为字符串类型，尝试设置的类型为 ${typeof value}`);
                        }
                        data.secret = value;
                    },
                },
                isActive: {
                    ...
                    set: (value: boolean) => {
                        if (typeof value !== 'boolean') {
                            throw new ArgumentError(`isActive属性必须为布尔值类型，尝试设置的类型为 ${typeof value}`);
                        }
                ...
                enableSSL: {
                    ...
                    set: (value: boolean) => {
                        if (typeof value !== 'boolean') {
                            throw new ArgumentError(`enableSSL属性必须为布尔值类型，尝试设置的类型为 ${typeof value}`);
                        }
