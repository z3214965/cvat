                ...
                position: {
                    ...
                    set: (value) => {
                        if (Array.isArray(value) || value.some((coord) => typeof coord !== 'number')) {
                            throw new ArgumentError(`应为数字数组，实际得到：${value}`);
                        }
                        data.position = value;
                    },
                },
    ...
    implementation: {
        ...
        value: async function implementation(this: Issue, data: RawCommentData) {
            if (typeof data !== 'object' || data === null) {
                throw new ArgumentError(`参数 "data" 必须为对象类型，实际得到："${data}"`);
            }
            if (typeof data.message !== 'string' || data.message.length < 1) {
                throw new ArgumentError(`评论消息必须为非空字符串，实际得到："${data.message}"`);
            }
    ...
    implementation: {
        ...
        value: async function implementation(user: User) {
            if (!(user instanceof User)) {
                throw new ArgumentError(`参数 "user" 必须是 User 类的实例，实际类型为："${typeof user}"`);
            }
