...
async function fetchData(url: string, requestConfig): Promise<{
    ...
    while (retry <= MAX_NO_PROGRESS_RETRIES) {
        ...
        try {
            ...
            if (response.status !== 206) {
                throw new Error(`非预期的响应状态码：${response.status}`);
            }
            ...
            if (!contentRange || contentRange.start !== receivedBytes || contentRange.total === null) {
                throw new Error('意外的 Content-Range 响应头');
            }
            ...
            if (receivedBytes > expectedSize) {
                throw new Error(`接收的字节数超出预期: ${receivedBytes}/${expectedSize}`);
            }
            ...
            if (receivedBytes < expectedSize) {
                ...
                throw new Error(`接收的字节数少于预期: ${receivedBytes}/${expectedSize}`);
            }
    ...
    throw new Error('已超出无进度的最大下载重试次数');
}
