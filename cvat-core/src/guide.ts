...
class AnnotationGuide {
    ...
    public set markdown(value: string) {
        if (typeof value !== 'string') {
            throw new ArgumentError(`Markdown 值必须为字符串类型，实际接收到的是 ${typeof value} 类型`);
        }
        this.#markdown = value;
    }
    ...
    implementation: {
        ...
        value: async function implementation(this: AnnotationGuide) {
            ...
            if (this.projectId === null && this.taskId === null) {
                throw new DataError('指南必须指定 projectId 或 taskId 中的一个');
            }

            if (this.projectId !== null && this.taskId !== null) {
                throw new DataError('指南不能同时包含 projectId 和 taskId');
            }
