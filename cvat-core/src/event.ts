...
export class Event {
    ...
    public validatePayload(): void {
        if (typeof this.payload !== 'object') {
            throw new ArgumentError('请求负载(Payload)必须是一个对象');
        }

        try {
            JSON.stringify(this.payload);
        } catch (error) {
            const message = `日志负载必须可序列化为JSON。${error.toString()}`;
...
class EventWithCount extends Event {
    public validatePayload(): void {
        ...
        if (!Number.isInteger(this.payload.count) || (this.payload.count as number) < 1) {
            const message = `"${this.scope}" 日志必须包含 "count" 字段，且该字段必须为正整数`;
            throw new ArgumentError(message);
        }
    }
}
...
class EventWithExceptionInfo extends Event {
    public validatePayload(): void {
        ...
        if (typeof this.payload.message !== 'string') {
            const message = `${this.scope} 日志必须包含 "message" 字段，且必须为字符串类型`;
            throw new ArgumentError(message);
        }

        if (typeof this.payload.filename !== 'string') {
            const message = `${this.scope} 日志必须包含 "filename" 字段，且必须为字符串类型`;
            throw new ArgumentError(message);
        }

        if (typeof this.payload.line !== 'number') {
            const message = `${this.scope} 日志必须包含 "line" 字段，且必须为数字类型`;
            throw new ArgumentError(message);
        }

        if (typeof this.payload.column !== 'number') {
            const message = `${this.scope} 日志必须包含 "column" 字段，且必须为数字类型`;
            throw new ArgumentError(message);
        }

        if (typeof this.payload.stack !== 'string') {
            const message = `${this.scope} 日志必须包含 "stack" 字段，且必须为字符串类型`;
            throw new ArgumentError(message);
        }
    }
