...
function validateNotEmptyString(value: string): void {
    if (typeof value !== 'string') {
        throw new ArgumentError(`值必须为字符串类型，检测到的实际类型：${typeof value}`);
    } else if (!value.trim().length) {
        throw new ArgumentError('值不能为空字符串');
    }
}
        ...
        Object.defineProperties(
            ...
            Object.freeze({
                ...
                description: {
                    ...
                    set: (value) => {
                        if (typeof value !== 'string') {
                            throw new ArgumentError('值必须为字符串');
                        }
                ...
                keyFile: {
                    ...
                    set: (file) => {
                        if (file instanceof File) {
                            ...
                            throw new ArgumentError(`必须为文件类型，检测到的实际类型：${typeof file}`);
                        }
                    },
                },
                connectionString: {
                ...
                providerType: {
                    ...
                    set: (key) => {
                        if (key !== undefined && !!CloudStorageProviderType[key]) {
                            ...
                            throw new ArgumentError('值必须是云存储提供商类型(CloudStorageProviderType)中的有效键值');
                        }
                    },
                },
                credentialsType: {
                    ...
                    set: (key) => {
                        if (key !== undefined && !!CloudStorageCredentialsType[key]) {
                            ...
                            throw new ArgumentError('值必须是云存储凭据类型(CloudStorageCredentialsType)中的有效键值');
                        }
                    },
                },
                specificAttributes: {
                    ...
                    set: (attributesValue) => {
                        if (typeof attributesValue === 'string') {
                            ...
                            if (!attrValues) {
                                throw new ArgumentError('值必须符合 key1=value1&key2=value2 格式');
                            }
                        ...
                        if (Array.isArray(manifests)) {
                            for (const elem of manifests) {
                                if (typeof elem !== 'string') {
                                    throw new ArgumentError('manifests数组中的每个元素都必须是字符串类型');
                                }
                            }
                            data.manifests = manifests;
                        } else {
                            throw new ArgumentError('值必须是一个数组');
                        }
                    },
                },
                prefix: {
