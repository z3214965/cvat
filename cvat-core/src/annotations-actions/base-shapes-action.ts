...
export async function run(
    ...
    try {
        ...
        // Iterate over frames
        const rawFrameNumbers =
            instance instanceof Job ? await instance.frames.frameNumbers() : range(0, instance.size);
        const frameNumbers = rawFrameNumbers.filter((frame) => frame >= frameFrom && frame <= frameTo);
        const totalFrames = frameNumbers.length;
        if (totalFrames === 0) {
            await showMessageWithPause('没有需要处理的帧', 100, 1500);
            return;
        }
        for (let frameIdx = 0; frameIdx < totalFrames; frameIdx++) {
            ...
            if (!frameData.deleted) {
                ...
                throttledOnProgress('操作正在进行', progress);
                if (cancelled()) {
                    return;
                }
            }
        ...
        await showMessageWithPause('正在提交已处理对象', 100, 1500);
        if (cancelled()) {
            return;
        }
...
export async function call(
    ...
    try {
        ...
        const exported = (await Promise.all(
            states.filter((state) => state.objectType === ObjectType.SHAPE).map((state) => state.export()),
        )) as SerializedShape[];
        const frameData = await Object.getPrototypeOf(instance).frames.get.implementation.call(instance, frame);
