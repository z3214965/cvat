...
export class CanvasModelImpl extends MasterImpl implements CanvasModel {
    ...
    public selectRegion(enable: boolean): void {
        if (enable && this.data.mode !== Mode.IDLE) {
            throw Error(`画布正忙，操作类型：${this.data.mode}`);
        }

        if (!enable && this.data.mode !== Mode.SELECT_REGION) {
            throw Error(`画布未处于区域选择模式，操作：${this.data.mode}`);
        }
    ...
    public dragCanvas(enable: boolean): void {
        if (enable && this.data.mode !== Mode.IDLE) {
            throw Error(`画布正忙，操作：${this.data.mode}`);
        }

        if (!enable && this.data.mode !== Mode.DRAG_CANVAS) {
            throw Error(`画布非拖动模式，操作：${this.data.mode}`);
        }
    ...
    public zoomCanvas(enable: boolean): void {
        ...
        if (!enable && this.data.mode !== Mode.ZOOM_CANVAS) {
            throw Error(`画布非缩放模式，操作：${this.data.mode}`);
        }
            ...
            .catch((exception: unknown): void => {
                if (typeof exception !== 'number') {
                    ...
                    if (exception instanceof Error) {
                        ...
                        this.data.exception = new Error('在获取图片数据时发生了未知错误');
                    }
    ...
    public draw(drawData: DrawData): void {
        const supportedShapes = ['rectangle', 'polygon', 'polyline', 'points', 'ellipse', 'cuboid', 'skeleton', 'mask'];
        ...
        if (drawData.enabled) {
            ...
            if (drawData.shapeType && !supportedShapes.includes(drawData.shapeType)) {
                throw new Error(`未实现"${drawData.shapeType}"类型的绘制方法`);
            }
    ...
    public edit(editData: MasksEditData | PolyEditData): void {
        ...
        if (
            ...
            throw Error('编辑过程中无法更新状态，请先完成当前编辑');
        }
    ...
    public join(joinData: JoinData): void {
        ...
        if ((this.data.joinData.enabled && joinData.enabled) || (!this.data.joinData.enabled && !joinData.enabled)) {
            return;
        }
