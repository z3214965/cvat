  ...
  constructor.prototype = {
    ...
    seek: function (index) {
      if (index < 0 || index > this.end) {
        error(`索引超出边界(边界范围：[0, ${this.end}]，索引：${index})。`);
      }
      this.pos = index;
    },
...
var MP4Reader = (function reader() {
  ...
  constructor.prototype = {
    ...
    readBox: function readBox(stream) {
      ...
      switch (box.type) {
        ...
        case 'mdat':
          ...
          assert (box.size >= 8, "暂不支持解析大型媒体数据。");
    ...
    read: function () {
      ...
      console.info(`流解析完成，耗时：${(new Date().getTime() - start)} 毫秒`);
    },
    traceSamples: function () {
      var video = this.tracks[1];
      var audio = this.tracks[2];

      console.info(`视频样本数：${video.getSampleCount()}`);
      console.info(`音频样本数：${audio.getSampleCount()}`);
      ...
      for (var i = 0; i < 100; i++) {
        ...
        if (vo < ao) {
          console.info(`视频样本 ${vi} 偏移量：${vo}，大小：${vs}`);
          vi ++;
        } else {
          console.info(`音频样本 ${ai} 偏移量：${ao}，大小：${as}`);
          ai ++;
        }
      }
    }
  ...
  constructor.prototype = {
    readAll: function(callback) {
      ...
      this.stream.readAll(null, function (buffer) {
        ...
        console.info(`MP4Player::readAll()，数据长度：${this.reader.stream.length}`);
