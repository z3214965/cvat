...
export default function TaskActionsItems(menuItemsData: MenuItemsData, taskMenuProps: unknown): MenuProps['items'] {
    ...
    menuItems.push([{
        ...
        label: withCount('更新标注', 'load_task_anno'),
    ...
    if (onOpenBugTracker) {
        menuItems.push([{
            ...
            label: withCount('打开问题追踪页', 'open_bug_tracker'),
    ...
    // menuItems.push([{
    //     key: 'view-analytics',
    //     label: withCount(t('common.view_analytics'), 'view-analytics', `/tasks/${taskId}/analytics`),
    //     disabled: isDisabled('view-analytics'),
    // }, 60]);

    // menuItems.push([{
    //     key: 'quality_control',
    //     label: withCount(t('common.quality_control'), 'quality_control', `/tasks/${taskId}/quality-control`),
    ...
    if (!projectId) {
        menuItems.push([{
            ...
            label: withCount('移动至项目', 'move_task_to_project'),
