    ...
    switch (request.status) {
        case RQStatus.FINISHED: {
            ...
            if (request.expiryDate && !type.includes('create') && !type.includes('import') && !exportToCloudStorage) {
                return (
                    <>
                        <Row>
                            {renderEllipsisText(`由 ${request.owner.username} 于 ${started} 发起`, 'secondary')}
                        </Row>
                        <Row>
            ...
            return (
                <>
                    <Row>
                        {renderEllipsisText(`由 ${request.owner.username} 在 ${started} 启动`, 'secondary')}
                    </Row>
                    <Row>
        ...
        case RQStatus.FAILED: {
            return (request.startedDate ? (
                <Row>
                    {renderEllipsisText(`由 ${request.owner.username} 于 ${started} 启动`, 'secondary')}
                </Row>
            ) : (
                <Row>
                    {renderEllipsisText(`由 ${request.owner.username} 于 ${created} 加入队列`, 'secondary')}
                </Row>
            ));
        }
